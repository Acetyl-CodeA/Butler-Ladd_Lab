...
class experimentClass(object):
    ...
    def experimentStartDialog(self):
        ...
        if self.readyUpdater():
            ...
            if runOrNot:
                ...
                if not self.__cam.updateGain(False):
                    msgbox.showerror(title='Error',message="Cannot update gain, see command line")
                    return
                if not self.__cam.updateExposure(False):
                ...
                self.__cam.gain = ct.c_double(self.__cam.gain)
                self.__cam.exposure = ct.c_double(self.__cam.exposure)
    ...
    def takePictureExp(self,timeForPicNum,numFrames,delay,frameInterval):
        '''
        The take (burst of) picture(s) function we use for experiments.
        
        timeForPicNum: Pass the time for the picture name so we can know 
        which picture burst each picture in this burst is associated with.
        
        type timeForPicNum: Float.
        
        numFrames: The number of frames to take in this burst.
        
        type numFrames: Int.
        
        delay: Time delay before we take the first frame.
        
        type delay: Float.
        
        frameInterval: Time delay between frames.
        
        type frameInterval: Float.
        
        return: None (generates a number of .txt files containing unprocessed
        pictures).
        '''
        
        #we need to put in the delay compensation, but we can't go below 0
        interval = max(0,frameInterval-self.antiDelay)
        time.sleep(max(0,(delay-self.antiDelay)))
        
        if self.__ncc.ShutterOpen:
            #leave it the open the entire time if it's open
            for j in range(int(numFrames)):
                name = self.outFolderPath + "/" + str(timeForPicNum) \
                    + "_picNumber_" + str(j+1) + '.txt'
                self.__cam.takePicturePython(name)
                self.rttsoe = time.time() - self.est
                print("Picture taken at relative time:",self.rttsoe)
                time.sleep(interval)
        else:
            for j in range(int(numFrames)):
                name = self.outFolderPath + "/" + str(timeForPicNum) \
